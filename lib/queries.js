...
export async function searchSchools(q, limit = 20, stateSlug = null, { category = null, mgmt = null } = {}) {
  ...
  if (/^\d+$/.test(term)) {
    const rows = await query(`
      ...
      FROM schools WHERE CAST(udise_code AS VARCHAR) = ? LIMIT 1
    `, [term]);
  ...
  // CockroachDB CAN use a B-tree index on school_name for prefix ILIKE 'term%'
  // This covers ~80% of real searches (people type school name)
  ...
  // User typed middle of name (e.g. "vidyalaya"). Still just one column.
  const containsPattern = `%${term}%`;
  const { clauses: c2, params: p2 } = buildFilters([containsPattern]);
  const nameResults = await query(`
    SELECT udise_code, school_name, village, district, state,
           school_category, national_mgmt, school_status, url, school_slug
    FROM schools
    WHERE school_name ILIKE ?${c2}
    ORDER BY school_name
    LIMIT ?
  `, [...p2, limit]);
  ...
  // Two separate indexed lookups, not a multi-column OR scan.
  const { clauses: c3, params: p3 } = buildFilters([containsPattern, containsPattern]);
  const broadResults = await query(`
    SELECT udise_code, school_name, village, district, state,
           school_category, national_mgmt, school_status, url, school_slug
    FROM schools
    WHERE (village ILIKE ? OR district ILIKE ?)${c3}
    ORDER BY school_name
    LIMIT ?
  `, [...p3, limit]);
