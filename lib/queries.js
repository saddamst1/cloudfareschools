...
export const getAllStates = cache(async function getAllStates() {
...
export const getState = cache(async function getState(stateSlug) {
  const slug = normalizeStateSlug(stateSlug);
  const row = await queryOne(`SELECT * FROM states WHERE state_slug = ? OR state_slug = ?`, [slug, stateSlug]);
  if (row) return row;
  return MOCK_STATES.find(s => s.state_slug === slug || s.state_slug === stateSlug) || null;
});

export const getStateDistricts = cache(async function getStateDistricts(stateSlug) {
  const slug = normalizeStateSlug(stateSlug);
  const rows = await query(`
    SELECT district_slug, district_name, state_name, total_schools, block_count, village_count
...
export const getStateCategoryCounts = cache(async function getStateCategoryCounts(stateSlug) {
  const slug = normalizeStateSlug(stateSlug);
...
export const getDistrict = cache(async function getDistrict(stateSlug, districtSlug) {
  ...
  if (mapping) {
    ...
    if (row && row.total_schools) return row;
  }
  const row = await queryOne(`
...
export const getDistrictBlocks = cache(async function getDistrictBlocks(stateSlug, districtSlug) {
  const mapping = getVirtualDistrictMapping(stateSlug, districtSlug);
  let rows = [];
  if (mapping) {
    const placeholders = mapping.slugs.map(() => '?').join(', ');
...
export const getDistrictCategoryCounts = cache(async function getDistrictCategoryCounts(stateSlug, districtSlug) {
  const mapping = getVirtualDistrictMapping(stateSlug, districtSlug);
  const qPromise = mapping ? (async () => {
    const placeholders = mapping.slugs.map(() => '?').join(', ');
    return await query(`
      SELECT school_category, COUNT(*) AS count FROM schools
...
export const getAdjacentDistricts = cache(async function getAdjacentDistricts(stateSlug, districtSlug) {
  if (!(await isDbAvailable())) {
    return [
...
export const getBlock = cache(async function getBlock(stateSlug, districtSlug, blockSlug) {
  return await queryOne(`
    SELECT * FROM blocks WHERE state_slug=? AND district_slug=? AND block_slug=?
  `, [stateSlug, districtSlug, blockSlug]);
});

export const getBlockVillages = cache(async function getBlockVillages(stateSlug, districtSlug, blockSlug) {
  return await query(`
...
export const getBlockSchools = cache(async function getBlockSchools(stateSlug, districtSlug, blockSlug, { page = 1, perPage = 24, category = null, mgmt = null } = {}) {
  ...
  const [totalRow, schools] = await Promise.all([
    queryOne(`SELECT COUNT(*) AS c FROM schools WHERE ${where}`, params).catch(() => ({ c: 0 })),
    query(`
...
export const getVillageSchools = cache(async function getVillageSchools(stateSlug, districtSlug, blockSlug, villageSlug) {
  return await query(`
...
export const getSchoolBySlug = cache(async function getSchoolBySlug(schoolSlug) {
...
export const getSchoolByUdise = cache(async function getSchoolByUdise(udiseCode) {
  if (!(await isDbAvailable())) {
...
export const getNearbySchools = cache(async function getNearbySchools(stateSlug, districtSlug, blockSlug, villageSlug, excludeUdise, limit = 6) {
  if (!(await isDbAvailable())) {
  ...
  return await query(`
    ...
    LIMIT ?
  `, [stateSlug, districtSlug, blockSlug, excludeUdise, limit]);
});

export const getDistrictStatsForSchool = cache(async function getDistrictStatsForSchool(stateSlug, districtSlug) {
