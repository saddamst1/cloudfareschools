...
// Helper: check if DB is accessible
let _dbAvailable = null;
async function isDbAvailable() {
  // Only cache positive results — don't permanently cache failures (cold start timeouts)
  if (_dbAvailable === true) return true;
  try {
    const result = await queryOne('SELECT 1 AS ok');
    _dbAvailable = result !== null ? true : null;
    return _dbAvailable === true;
  } catch {
    _dbAvailable = null; // Allow retry on next request
    return false;
  }
}
...
export async function getHomepageStats() {
  if (!(await isDbAvailable())) return MOCK_STATS;
  return await queryOne(`
    SELECT COUNT(*) AS total_schools,
           COUNT(DISTINCT state_slug) AS total_states,
           COUNT(DISTINCT district_slug) AS total_districts,
           COUNT(DISTINCT village_slug) AS total_villages
    FROM schools WHERE school_status = 'Operational'
  `) || MOCK_STATS;
}
...
export async function getState(stateSlug) {
  if (!(await isDbAvailable())) {
    return MOCK_STATES.find(s => s.state_slug === stateSlug) || null;
  }
  return await queryOne(`SELECT * FROM states WHERE state_slug = ?`, [stateSlug]);
}

export async function getStateDistricts(stateSlug) {
  if (!(await isDbAvailable())) {
    return stateSlug === 'uttar-pradesh' ? MOCK_DISTRICTS_UP : [];
  }
  return await query(`
    ...
    FROM districts WHERE state_slug = ? ORDER BY total_schools DESC
  `, [stateSlug]);
}
...
export async function getStateCategoryCounts(stateSlug) {
  ...
  return await query(`
    ...
    WHERE state_slug = ? GROUP BY school_category ORDER BY count DESC
  `, [stateSlug]);
}
...
export async function getStateMgmtCounts(stateSlug) {
  ...
  return await query(`
    ...
    WHERE state_slug = ? GROUP BY national_mgmt ORDER BY count DESC
  `, [stateSlug]);
}
...
export async function getDistrictBlocks(stateSlug, districtSlug) {
  ...
  return await query(`
    SELECT block_slug, block_name, district_name, state_name, total_schools, village_count
...
export async function getAdjacentDistricts(stateSlug, districtSlug) {
  ...
  return await query(`
    SELECT district_slug, district_name, state_slug, total_schools, block_count
    FROM districts
    WHERE state_slug = ? AND district_slug != ? LIMIT 4
  `, [stateSlug, districtSlug]);
}
...
export async function getBlockSchools(stateSlug, districtSlug, blockSlug, { page = 1, perPage = 24, category = null, mgmt = null } = {}) {
  ...
  const schools  = await query(`
    SELECT udise_code, school_name, village, village_slug, school_category,
...
export async function getVillageSchools(stateSlug, districtSlug, blockSlug, villageSlug) {
  ...
  return await query(`
    SELECT udise_code, school_name, school_category, school_type,
...
export async function searchSchools(q, limit = 20, stateSlug = null, { category = null, mgmt = null } = {}) {
  ...
  if (!(await isDbAvailable())) {
    ...
    return MOCK_SCHOOLS.filter(s => {
      ...
      return matchText && matchCat && matchMgmt && matchState;
    }).slice(0, limit);
  }
  ...
  if (/^\d+$/.test(term)) {
    ...
    const rows = await query(`
      SELECT udise_code, school_name, village, district, state,
             school_category, national_mgmt, school_status, url, school_slug
  ...
  const prefixPattern = `${term.toLowerCase()}%`;
  const { clauses: c1, params: p1 } = buildFilters([prefixPattern]);
  const prefixResults = await query(`
    SELECT udise_code, school_name, village, district, state,
           school_category, national_mgmt, school_status, url, school_slug
    FROM schools
    WHERE lower(school_name) LIKE ?${c1}
    ORDER BY school_name
    LIMIT ?
  `, [...p1, limit]);

  if (prefixResults.length >= 5) return prefixResults;

  // ── Tier 2: Contains match on school_name ───────────────────────────────────
  // User typed middle of name (e.g. "vidyalaya").
  // Use the GIN index with index hint on Postgres/CockroachDB
  const containsPattern = `%${term}%`;
  const isPg = isPostgres();
  const tableName = isPg ? 'schools@idx_schools_trgm_direct' : 'schools';
  const nameOperator = isPg ? 'school_name ILIKE ?' : 'lower(school_name) LIKE ?';
  const namePattern = isPg ? containsPattern : containsPattern.toLowerCase();

  const { clauses: c2, params: p2 } = buildFilters([namePattern]);
  const nameResults = await query(`
    SELECT udise_code, school_name, village, district, state,
           school_category, national_mgmt, school_status, url, school_slug
    FROM ${tableName}
    WHERE ${nameOperator}${c2}
    ORDER BY school_name
    LIMIT ?
  `, [...p2, limit]);

  if (nameResults.length >= 3) return nameResults;

  // ── Tier 3: Broad search — village OR district ───────────────────────────────
  // Only reached when school_name search returns very few results.
  // We execute parallel queries using index hints on Postgres to avoid OR scans
  let broadResults = [];
  if (isPg) {
    ...
    const [villageRes, districtRes] = await Promise.all([
      query(`
        SELECT udise_code, school_name, village, district, state,
               school_category, national_mgmt, school_status, url, school_slug
        FROM schools@idx_schools_trgm_village
      ...
      query(`
        SELECT udise_code, school_name, village, district, state,
               school_category, national_mgmt, school_status, url, school_slug
        FROM schools@idx_schools_trgm_district
