...
let _lastDbError = 'No error recorded yet';
...
async function initClient() {
  ...
  _initPromise = (async () => {
    const rawDbUrl = process.env.DATABASE_URL || 'postgresql://schoolsv1:taxzmVDYud4CswwoUYvuxg@sound-python-30115.j77.aws-ap-south-1.cockroachlabs.cloud:26257/defaultdb?sslmode=no-verify';
    const dbUrl = rawDbUrl.includes('sslmode=') ? rawDbUrl : `${rawDbUrl}?sslmode=no-verify`;
    ...
    try {
      ...
      _lastDbError = 'CONNECT ERROR: ' + (e.stack || e.message || String(e));
...
export async function query(sql, params = []) {
  try {
    ...
    _lastDbError = 'QUERY ERROR: ' + (e.stack || e.message || String(e));
    console.error('[DB Query Error]', e.message, '| SQL:', sql.slice(0, 100));
    // Self-heal ONLY on true connection loss (DO NOT reset pool on slow query statement timeouts!)
    if (e.message && (e.message.includes('ECONNRESET') || e.message.includes('connection terminated') || e.message.includes('disestablished'))) {
      _pool = null;
      _initPromise = null;
    }
    return [];
