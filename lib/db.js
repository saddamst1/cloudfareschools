  ...
  if (dbUrl.startsWith('postgres:') || dbUrl.startsWith('postgresql:')) {
    try {
      ...
      _client = new Pool({
        ...
        // Vercel functions are stateless — each invocation gets 1 connection.
        // max=5 prevents connection exhaustion on CockroachDB free tier (limit: 25).
        max: parseInt(process.env.PG_POOL_MAX || '5', 10),
        idleTimeoutMillis: 10000,
        connectionTimeoutMillis: 8000,
        ssl: sslConfig,
        // Kill runaway queries after 10s — protects against slow search scans
