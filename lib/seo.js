...
export function getSchoolMeta(school, lang = 'en') {
  ...
  // Fix 1: English — new click-worthy title and description
  const enTitle = isClosed
    ? `${name} — ${village}, ${dist} (Permanently Closed) | SchoolsPedia`
    : `${name} — ${village}, ${dist} | ${state} ${mgmtLabel} ${cat} School | SchoolsPedia`;

  const enDesc = isClosed
    ? `${name} is permanently closed. It was located in ${village}, ${dist}, ${state}.`
    : `${name} is a ${typeLabel} ${cat} ${mgmtLabel} school in ${village}, ${dist}, ${state}.` +
      (estYear ? ` Est. ${estYear}.` : '') +
      ` Get 2025-26 admission info, UDISE code ${udise}, nearby schools & RTE seats — free on SchoolsPedia.`;

  return {
    title: enTitle,
    description: enDesc,
    keywords: `${name}, UDISE ${udise}, ${name} ${dist}, ${village} school ${dist}, ${cat} school ${dist} ${state}, ${mgmtLabel} school ${village}`,
    alternates,
    openGraph: {
      title: `${name} — ${village}, ${dist}${isClosed ? ' (Closed)' : ''}`,
      description: isClosed
