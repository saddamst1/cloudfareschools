...
export function getStateMeta(state, lang = 'en') {
  ...
  if (lang === 'hi') {
    return {
      ...
      alternates: { canonical: `https://www.schoolspedia.in/hi/schools/${state.state_slug}` },
      openGraph: {
...
export function getDistrictMeta(district, lang = 'en') {
  ...
  if (lang === 'hi') {
    return {
      ...
      alternates: { canonical: `https://www.schoolspedia.in/hi/schools/${district.state_slug}/${district.district_slug}` },
      openGraph: {
...
export function getBlockMeta(block, lang = 'en') {
  ...
  if (lang === 'hi') {
    return {
      ...
      alternates: { canonical: `https://www.schoolspedia.in/hi/schools/${block.state_slug}/${block.district_slug}/${block.block_slug}` },
    };
  }
...
export function getVillageMeta(village, stateSlug, districtSlug, blockSlug, schoolCount, lang = 'en') {
  ...
  if (lang === 'hi') {
    return {
      ...
      alternates: { canonical: `https://www.schoolspedia.in/hi/schools/${stateSlug}/${districtSlug}/${blockSlug}/${village}` },
    };
  }
...
export function getSchoolMeta(school, lang = 'en') {
  ...
  if (lang === 'hi') {
    return {
      ...
      description: `${name} ${cat} स्कूल है, जो ${village} गाँव, ${dist} जिला, ${state} में स्थित है। UDISE कोड: ${udise} है। प्रबंधन: ${mgmt}। RTE प्रवेश और स्कूल की अन्य जानकारी प्राप्त करें।`,
      keywords: `${name}, UDISE ${udise}, ${village} school, ${dist} school, ${cat} school ${state}`,
      alternates: { canonical: `https://www.schoolspedia.in/hi${school.url}` },
      openGraph: {
        title: `${name} — UDISE ${udise}`,
        description: `${cat} स्कूल, ${village}, ${dist}, ${state}`,
