...
// Utility: Convert slug/lowercase string to Title Case
...
export function getHomepageMeta() {
  return {
    title: 'SchoolsPedia — School Directory India 2026 | 16.5 Lakh+ Schools & UDISE Codes',
    description: 'Find any school in India by name, UDISE code, village, block or district. 1,653,159 schools from UDISE+ across 36 states. Official government & private school list.',
    keywords: 'schools in India 2026, UDISE code, school directory India, government schools, private schools, RTE seats, UDISE+',
    openGraph: {
      title: 'SchoolsPedia — Find Any School in India 2026',
      description: 'India\'s most comprehensive school directory. 16.5 Lakh+ schools across 36 states. UDISE codes, RTE admission guides, demographics — all free.',
...
export function getStateMeta(state, lang = 'en') {
  ...
  if (lang === 'hi') {
    return {
      title: `${stateName} में स्कूल सूची 2026 (PDF Download) — ${n}+ स्कूल, UDISE कोड | SchoolsPedia`,
      description: `${stateName} में ${n}+ स्कूलों की आधिकारिक 2026 सूची और PDF Download। ${state.district_count} जिलों और ${state.block_count} ब्लॉकों में सरकारी, निजी, KVS और NVS स्कूलों की जानकारी UDISE कोड के साथ।`,
      alternates,
      openGraph: {
        title: `${stateName} में स्कूल (PDF Download) | SchoolsPedia`,
  ...
  return {
    title: `Schools in ${state.state_name} (2026 List PDF Download) — ${n}+ Schools | SchoolsPedia`,
    description: `Browse and download PDF list of ${n}+ schools in ${state.state_name} (2026 updated) across ${state.district_count} districts. Find government, private, KVS, and NVS schools by district, block, or village.`,
    alternates,
    openGraph: {
      title: `Schools in ${state.state_name} 2026 (PDF List) | SchoolsPedia`,
...
export function getDistrictMeta(district, lang = 'en') {
  ...
  if (lang === 'hi') {
    return {
      title: `${distName} जिला (${stateName}) स्कूल सूची 2026 (PDF Download) — UDISE कोड | SchoolsPedia`,
      description: `${stateName} के ${distName} जिले में ${n}+ स्कूल PDF Download। ${district.block_count} ब्लॉकों में सरकारी और निजी स्कूलों की 2026 सूची, UDISE कोड की पूरी जानकारी।`,
      alternates,
      openGraph: {
        title: `${distName} जिले में स्कूल (PDF List) | SchoolsPedia`,
  ...
  return {
    title: `Schools in ${district.district_name} District 2026 List (PDF Download) — UDISE Codes | SchoolsPedia`,
    description: `Find & download PDF list of ${n}+ schools in ${district.district_name} district, ${district.state_name} (2026 updated). Browse ${district.block_count} blocks, government and private schools, UDISE codes.${lit}`,
    alternates,
    openGraph: {
      title: `Schools in ${district.district_name} District 2026 (PDF Download) | SchoolsPedia`,
...
export function getBlockMeta(block, lang = 'en') {
  ...
  if (lang === 'hi') {
    return {
      title: `${blockName} ब्लॉक, ${distName} (${stateName}) स्कूल सूची 2026 (PDF Download) | SchoolsPedia`,
      description: `${stateName} के ${distName} जिले के ${blockName} ब्लॉक में ${n}+ स्कूलों की 2026 PDF Download सूची। ${block.village_count} गाँवों में UDISE कोड और विस्तृत जानकारी।`,
      alternates,
    };
  }

  return {
    title: `Schools in ${block.block_name} Block, ${block.district_name} (PDF Download 2026) | SchoolsPedia`,
    description: `Browse & download PDF list of ${n}+ schools in ${block.block_name} block, ${block.district_name}, ${block.state_name} (2026 list). ${block.village_count} villages covered with UDISE codes.`,
    alternates,
  };
}
...
export function getSchoolMeta(school, lang = 'en') {
  ...
  // Fix 4: Proper Title Case for district and state — no more lowercase "patna"
  const dist  = toTitleCase(t(school.district_slug, lang) || school.district);
  const state = toTitleCase(t(school.state_slug, lang) || school.state);
  ...
  // Fix 1: Labels for Govt/Private and school type (Boys/Girls/Co-ed)
  const isGovt = school.national_mgmt?.includes('Department') || school.national_mgmt?.includes('Local Body');
  const mgmtLabel = isGovt ? 'Government' : 'Private';
  const typeLabel = school.school_type === 'Boys' ? 'Boys' : school.school_type === 'Girls' ? 'Girls' : 'Co-ed';
  const estYear = Number(school.establishment_year) > 1800 ? school.establishment_year
    : Number(school.year_established) > 1800 ? school.year_established : null;
  ...
  if (lang === 'hi') {
    // Hindi: improved title with location clarity
    const hiTitle = isClosed
      ? `${name} — ${village}, ${dist} (बंद) | SchoolsPedia`
      : `${name} — ${village}, ${dist} | ${state} ${cat} स्कूल | SchoolsPedia`;
    const hiDesc = isClosed
      ? `${name} स्कूल स्थायी रूप से बंद (Permanently Closed) हो चुका है। यह ${village} गाँव, ${dist} जिला, ${state} में स्थित था।`
      : `${name} ${dist} जिले के ${village} गाँव का एक ${typeLabel === 'Girls' ? 'बालिका' : typeLabel === 'Boys' ? 'बालक' : 'सह-शिक्षा'} ${cat} ${mgmtLabel === 'Government' ? 'सरकारी' : 'निजी'} स्कूल है।` +
        (estYear ? ` स्थापना: ${estYear}।` : '') +
        ` UDISE: ${udise}। 2025-26 प्रवेश जानकारी, पास के स्कूल और RTE सीटें — SchoolsPedia पर मुफ़्त।`;
    return {
      title: hiTitle,
      description: hiDesc,
      keywords: `${name}, UDISE ${udise}, ${village} स्कूल, ${dist} स्कूल, ${cat} स्कूल ${state}`,
      alternates,
      openGraph: {
        title: `${name} — ${village}, ${dist}${isClosed ? ' (बंद)' : ''}`,
        description: isClosed
          ? `यह स्कूल स्थायी रूप से बंद हो चुका है।`
  ...
  // English — keyword-rich title: School Name | Village District | UDISE | Type School
  // Leading with exact school name ensures exact-match query ranking
  const enTitle = isClosed
    ? `${name} — ${village}, ${dist} (Permanently Closed) | UDISE ${udise} | SchoolsPedia`
    : `${name} — ${village}, ${dist} | UDISE ${udise} | ${state} ${mgmtLabel} ${cat} School`;

  const enDesc = isClosed
    ? `${name} (UDISE: ${udise}) is permanently closed. It was a ${cat} school in ${village}, ${dist}, ${state}.`
    : `${name} is a ${typeLabel} ${mgmtLabel} ${cat} school in ${village}, ${dist}, ${state}.` +
      (estYear ? ` Established ${estYear}.` : '') +
      ` UDISE code: ${udise}. 2026 admission info, RTE seats, nearby schools & school details — free on SchoolsPedia.`;

  return {
    title: enTitle,
    description: enDesc,
    keywords: `${name}, UDISE ${udise}, ${name} ${village}, ${name} ${dist}, ${village} ${cat} school, ${mgmtLabel} school ${village} ${dist}, ${cat} school ${dist} ${state}`,
    alternates,
    openGraph: {
      title: `${name} | UDISE ${udise} — ${village}, ${dist}${isClosed ? ' (Closed)' : ''}`,
      description: isClosed
        ? `This school is permanently closed.`
        : `${typeLabel} ${cat} ${mgmtLabel} school in ${village}, ${dist}, ${state}`,
...
export function schoolSchema(school, districtStats, reviewStats = null) {
  ...
  const schema = {
    ...
    // Fix 3: description field in schema.org for rich results eligibility
    description: `${school.school_name} is a ${school.school_category} school in ${school.village}, ${school.district}, ${school.state}. UDISE code: ${udise}. Management: ${school.national_mgmt || 'N/A'}.`,
    // Fix 3: sameAs — link to UDISE+ official source
    sameAs: ['https://udiseplus.gov.in/'],
  };
  ...
  // Fix 3: numberOfStudents — enrollment signal for Google
  const totalStudents = Number(school.total_students || school.enrollment_total);
  if (totalStudents > 0) {
...
export function breadcrumbSchema(items) {
  const validItems = (items || [])
    .map(item => {
      ...
      // If name is empty, attempt to resolve a readable name from the URL slug
      if (!name && item.url) {
        const parts = item.url.split('/').filter(Boolean);
        if (parts.length > 0) {
          const lastPart = parts[parts.length - 1];
          name = lastPart.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
        }
      }
      return {
        name: name || 'Page',
